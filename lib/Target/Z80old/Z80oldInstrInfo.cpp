//===-- Z80oldInstrInfo.cpp - Z80old Instruction Information --------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the Z80old implementation of the TargetInstrInfo class.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "z80old-instr-info"

#define GET_INSTRINFO_CTOR_DTOR
#include "Z80oldGenInstrInfo.inc"

// Pin the vtable to this file.
void Z80oldInstrInfo::anchor() {}

Z80oldInstrInfo::Z80oldInstrInfo(Z80oldSubtarget &STI)
  : Z80oldGenInstrInfo(Z80old::ADJCALLSTACKDOWN16, Z80old::ADJCALLSTACKUP16),
    Subtarget(STI), RI(STI.getTargetTriple()) {
}

int Z80oldInstrInfo::getSPAdjust(const MachineInstr &MI) const {
  switch (MI.getOpcode()) {
  case Z80old::POP16r:
  case Z80old::POP16AF:
    return 2;
  case Z80old::PUSH16r:
  case Z80old::PUSH16AF:
    return -2;
  }
...
static bool isIndex(const MachineOperand &MO, const MCRegisterInfo &RI) {
  ...
  if (MO.isReg())
    for (unsigned IndexReg : Z80old::IR16RegClass)
...
unsigned Z80oldInstrInfo::getInstSizeInBytes(const MachineInstr &MI) const {
  auto TSFlags = MI.getDesc().TSFlags;
  // 1 byte for opcode
  unsigned Size = 1;
  // 1 byte if we need a suffix
  // FIXME: create an operand for the suffix
  switch (TSFlags >> Z80oldII::ImmSizeShift & Z80oldII::ImmSizeMask) {
  case 2:
  ...
  // prefix byte(s)
  unsigned Prefix = TSFlags >> Z80oldII::PrefixShift & Z80oldII::PrefixMask;
  bool HasPrefix;
  if (TSFlags & Z80oldII::IndexedIndexPrefix) {
    Size += HasPrefix = isIndex(MI.getOperand(Prefix), getRegisterInfo());
  } else
    switch (Prefix) {
    case Z80oldII::NoPrefix:
      HasPrefix = false;
      break;
    case Z80oldII::CBPrefix:
    case Z80oldII::DDPrefix:
    case Z80oldII::EDPrefix:
    case Z80oldII::FDPrefix:
      Size += 1;
      HasPrefix = true;
      break;
    case Z80oldII::DDCBPrefix:
    case Z80oldII::FDCBPrefix:
      Size += 2;
      HasPrefix = true;
      break;
    case Z80oldII::AnyIndexPrefix:
      Size += HasPrefix = hasIndex(MI, getRegisterInfo());
      break;
    }
  // immediate byte(s)
  if (TSFlags & Z80oldII::HasImm) {
    unsigned ImmSize = TSFlags >> Z80oldII::ImmSizeShift & Z80oldII::ImmSizeMask;
    if (!ImmSize) {
      ImmSize = 2;
    }
    Size += ImmSize;
...
Z80old::CondCode Z80old::GetOppositeBranchCondition(Z80old::CondCode CC) {
  return Z80old::CondCode(CC ^ 1);
}

bool Z80oldInstrInfo::isUnpredicatedTerminator(const MachineInstr &MI) const {
...
bool Z80oldInstrInfo::analyzeBranch(MachineBasicBlock &MBB,
  ...
  while (I != MBB.begin()) {
    ...
    Z80old::CondCode BranchCode = Z80old::CondCode(I->getOperand(1).getImm());
    ...
    if (Cond.empty()) {
      ...
      if (AllowModify && UnCondBrIter != MBB.end() &&
        ...
        BuildMI(MBB, UnCondBrIter, MBB.findDebugLoc(I), get(Z80old::JQCC))
        .addMBB(UnCondBrIter->getOperand(0).getMBB()).addImm(BranchCode);
        BuildMI(MBB, UnCondBrIter, MBB.findDebugLoc(I), get(Z80old::JQ))
        .addMBB(TargetBB);
...
unsigned Z80oldInstrInfo::removeBranch(MachineBasicBlock &MBB,
  ...
  while (I != MBB.begin()) {
    ...
    if (I->getOpcode() != Z80old::JQ &&
        I->getOpcode() != Z80old::JQCC) {
      break;
    }
    // Remove the branch.
    I->eraseFromParent();
...
unsigned Z80oldInstrInfo::insertBranch(MachineBasicBlock &MBB,
                                    MachineBasicBlock *TBB,
                                    MachineBasicBlock *FBB,
  ...
  assert(Cond.size() <= 1 && "Z80old branch conditions have one component!");
  assert(!BytesAdded && "code size not handled");

  if (Cond.empty()) {
    // Unconditional branch?
    assert(!FBB && "Unconditional branch with multiple successors!");
    BuildMI(&MBB, DL, get(Z80old::JQ)).addMBB(TBB);
    return 1;
  }

  // Conditional branch.
  unsigned Count = 0;
  BuildMI(&MBB, DL, get(Z80old::JQCC)).addMBB(TBB).addImm(Cond[0].getImm());
  ++Count;

  // If FBB is null, it is implied to be a fall-through block.
  if (FBB) {
    // Two-way Conditional branch. Insert the second branch.
    BuildMI(&MBB, DL, get(Z80old::JQ)).addMBB(FBB);
    ++Count;
  }
  return Count;
}

bool Z80oldInstrInfo::
reverseBranchCondition(SmallVectorImpl<MachineOperand> &Cond) const {
  assert(Cond.size() == 1 && "Invalid Z80old branch condition!");
  Z80old::CondCode CC = static_cast<Z80old::CondCode>(Cond[0].getImm());
  Cond[0].setImm(GetOppositeBranchCondition(CC));
  return false;
}

bool Z80old::splitReg(
  ...
  case 1:
    RC = Z80old::RR8RegClassID;
    LoOpc = HiOpc = Opc8;
    LoIdx = HiIdx = Z80old::NoSubRegister;
    HiOff = 0;
    return false;
#if 0
  case 2:
    RC = Z80old::R16RegClassID;
    LoOpc = HiOpc = Opc8;
    LoIdx = Z80old::sub_low;
    HiIdx = Z80old::sub_high;
    HiOff = 1;
    return true;
#endif // 0
  }
...
bool Z80oldInstrInfo::canExchange(unsigned RegA, unsigned RegB) const {
  // The only regs that can be directly exchanged are DE and HL, in any order.
  bool DE = false, HL = false;
  for (unsigned Reg : {RegA, RegB}) {
    if (RI.isSubRegisterEq(Z80old::DE, Reg)) {
      DE = true;
    } else if (RI.isSubRegisterEq(Z80old::HL, Reg)) {
      HL = true;
    }
  }
  return DE && HL;
...
void Z80oldInstrInfo::copyPhysReg(MachineBasicBlock &MBB,
  ...
  /*for (auto Regs : {std::make_pair(DstReg, &SrcReg),
    ...
    if (Z80old::RR8RegClassID.contains(Regs.first) &&
        (Z80old::R16RegClass.contains(*Regs.second) ||
         Z80old::R24RegClass.contains(*Regs.second)))
      *Regs.second = RI.getSubReg(*Regs.second, Z80old::sub_low);
  }*/
  // Identity copy.
  if (DstReg == SrcReg) {
    return;
  }
  if (Z80old::RR8RegClass.contains(DstReg, SrcReg)) {
    // Byte copy.
    if (Z80old::GR8RegClass.contains(DstReg, SrcReg)) {
      // Neither are index registers.
      BuildMI(MBB, MI, DL, get(Z80old::LD8gg), DstReg)
      .addReg(SrcReg, getKillRegState(KillSrc));
#if 0 // $MS: Don't use index half registers
    } else if (Z80old::I8RegClass.contains(DstReg, SrcReg)) {
      assert(Subtarget.hasIndexHalfRegs() && "Need  index half registers");
      // Both are index registers.
      if (Z80old::X8RegClass.contains(DstReg, SrcReg)) {
        BuildMI(MBB, MI, DL, get(Z80old::LD8xx), DstReg)
        .addReg(SrcReg, getKillRegState(KillSrc));
      } else if (Z80old::Y8RegClass.contains(DstReg, SrcReg)) {
        BuildMI(MBB, MI, DL, get(Z80old::LD8yy), DstReg)
        .addReg(SrcReg, getKillRegState(KillSrc));
      } else {
        // We are copying between different index registers, so we need to use
        // an intermediate register.
        BuildMI(MBB, MI, DL, get(Z80old::PUSH16AF));
        BuildMI(MBB, MI, DL, get(Z80old::X8RegClass.contains(SrcReg) ? Z80old::LD8xx
                                 : Z80old::LD8yy),
                Z80old::A).addReg(SrcReg, getKillRegState(KillSrc));
        BuildMI(MBB, MI, DL, get(Z80old::X8RegClass.contains(DstReg) ? Z80old::LD8xx
                                 : Z80old::LD8yy),
                DstReg).addReg(Z80old::A);
        BuildMI(MBB, MI, DL, get(Z80old::POP16AF));
      }
    } else {
      assert(Subtarget.hasIndexHalfRegs() && "Need  index half registers");
      ...
      for (unsigned *Reg : {&DstReg, &SrcReg}) {
        switch (*Reg) {
        case Z80old::H: *Reg = Z80old::D; NeedEX = true; break;
        case Z80old::L: *Reg = Z80old::E; NeedEX = true; break;
        }
      }
      unsigned ExOpc = Z80old::EX16DE;
      if (NeedEX) {
        ...
        if (MI != MBB.begin() && std::prev(MI)->getOpcode() == ExOpc) {
          ...
          for (unsigned Reg : { Z80old::DE, Z80old::HL })
            ExMI.findRegisterUseOperand(Reg)->setIsUndef();
        }
      }
      BuildMI(MBB, MI, DL,
              get(Z80old::X8RegClass.contains(DstReg, SrcReg) ? Z80old::LD8xx
                  : Z80old::LD8yy),
  ...
  // Copies to SP.
  if (DstReg == Z80old::SPS) {
    assert((Z80old::AIR16RegClass.contains(SrcReg) || SrcReg == Z80old::DE) &&
           "Unimplemented");
    if (SrcReg == Z80old::DE)
      BuildMI(MBB, MI, DL, get(Z80old::EX16DE))
      .addReg(DstReg, RegState::ImplicitDefine)
      .addReg(SrcReg, RegState::ImplicitKill);
    BuildMI(MBB, MI, DL, get(Z80old::LD16SP))
    .addReg(SrcReg, getKillRegState(KillSrc));
    if (SrcReg == Z80old::DE)
      BuildMI(MBB, MI, DL, get(Z80old::EX16DE))
      .addReg(DstReg, RegState::ImplicitDefine)
  ...
  // Copies from SP.
  if (SrcReg == Z80old::SPS) {
    assert((Z80old::AIR16RegClass.contains(DstReg) || DstReg == Z80old::DE) &&
           "Unimplemented");
    if (DstReg == Z80old::DE)
      BuildMI(MBB, MI, DL, get(Z80old::EX16DE))
      .addReg(DstReg, RegState::ImplicitDefine)
      .addReg(SrcReg, RegState::ImplicitKill);
    BuildMI(MBB, MI, DL, get(Z80old::LD16ri),
            DstReg).addImm(0);
    BuildMI(MBB, MI, DL, get(Z80old::ADD16SP),
  ...
  //if (Is24Bit == Subtarget.is24Bit()) // $TODO ??? should always be true
  {
    ...
    bool IsSrcIndexReg = Z80old::IR16RegClass.contains(SrcReg);
    ...
    bool IsDstIndexReg = Z80old::IR16RegClass.contains(DstReg);
    ...
    if ((NumIndexRegs == 1 && OptSize) ||
      ...
      BuildMI(MBB, MI, DL, get(Z80old::PUSH16r))
      .addReg(SrcReg, getKillRegState(KillSrc));
      BuildMI(MBB, MI, DL, get(Z80old::POP16r), DstReg);
      return;
    }
  }
  // Otherwise, implement as two copies. A 16-bit copy should copy high and low
  // 8 bits separately.
  assert(Z80old::R16RegClass.contains(DstReg, SrcReg) && "Unknown copy width");
  unsigned SubLo = Z80old::sub_low;
  unsigned SubHi = Z80old::sub_high;
  ...
  if (DstLoSrcHiOverlap && SrcLoDstHiOverlap) {
    ...
    if (canExchange(DstLoReg, SrcLoReg)) {
      BuildMI(MBB, MI, DL, get(Subtarget.is24Bit() ? Z80old::EX24DE : Z80old::EX16DE))
        .addReg(DstReg, RegState::ImplicitDefine)
        .addReg(SrcReg, RegState::ImplicitKill);
    } else if ((OtherReg = DstLoReg, RI.isSubRegisterEq(Z80old::UHL, SrcLoReg)) ||
               (OtherReg = SrcLoReg, RI.isSubRegisterEq(Z80old::UHL, DstLoReg))) {
      BuildMI(MBB, MI, DL, get(Subtarget.is24Bit() ? Z80old::PUSH24r : Z80old::PUSH16r))
        .addReg(OtherReg, RegState::Kill);
      BuildMI(MBB, MI, DL, get(Subtarget.is24Bit() ? Z80old::EX24SP : Z80old::EX16SP));
      BuildMI(MBB, MI, DL, get(Subtarget.is24Bit() ? Z80old::POP24r : Z80old::POP16r),
              OtherReg);
    } else {
      BuildMI(MBB, MI, DL, get(Subtarget.is24Bit() ? Z80old::PUSH24r : Z80old::PUSH16r))
...
void Z80oldInstrInfo::storeRegToStackSlot(MachineBasicBlock &MBB,
                                       MachineBasicBlock::iterator MI,
  ...
  bool Split =
    Z80old::splitReg(TRI->getSpillSize(*TRC), Z80old::LD8or, // Z80old::LD16or,
...
void Z80oldInstrInfo::loadRegFromStackSlot(MachineBasicBlock &MBB,
                                        MachineBasicBlock::iterator MI,
  ...
  unsigned RC, LoOpc, LoIdx, HiOpc, HiIdx, HiOff;
  bool Split =
    Z80old::splitReg(TRI->getSpillSize(*TRC), Z80old::LD8ro, // Z80old::LD16ro,
                  RC, LoOpc, LoIdx, HiOpc, HiIdx, HiOff);
  MachineInstrBuilder LoMIB =
...
bool Z80oldInstrInfo::isFrameOperand(const MachineInstr &MI, unsigned int Op,
...
unsigned Z80oldInstrInfo::isLoadFromStackSlot(const MachineInstr &MI,
                                           int &FrameIndex) const {
...
unsigned Z80oldInstrInfo::isStoreToStackSlot(const MachineInstr &MI,
                                          int &FrameIndex) const {
...
bool Z80oldInstrInfo::isReallyTriviallyReMaterializable(const MachineInstr &MI,
...
void Z80oldInstrInfo::reMaterialize(MachineBasicBlock &MBB,
                                 MachineBasicBlock::iterator I,
                                 unsigned DstReg, unsigned SubIdx,
                                 const MachineInstr &Orig,
                                 const TargetRegisterInfo &TRI) const {
  if (!Orig.modifiesRegister(Z80old::F, &TRI) ||
      MBB.computeRegisterLiveness(&TRI, Z80old::F, I) ==
...
bool Z80oldInstrInfo::expandPostRAPseudo(MachineInstr &MI) const {
  ...
  //.hasAttribute(AttributeList::FunctionIndex, Attribute::OptimizeForSize);
  LLVM_DEBUG(dbgs() << "\nZ80oldInstrInfo::expandPostRAPseudo:"; MI.dump());
  switch (unsigned Opc = MI.getOpcode()) {
  default:
    return false;
  case Z80old::RCF:
    MI.setDesc(get(Z80old::OR8ar));
    MIB.addReg(Z80old::A, RegState::Undef);
    break;
  case Z80old::LD8r0:
    if (MI.getOperand(0).getReg() == Z80old::A) {
      MI.setDesc(get(Z80old::XOR8ar));
      MI.getOperand(0).setIsUse();
      MI.getOperand(0).setIsUndef();
      MIB.addReg(Z80old::A, RegState::ImplicitDefine);
    } else {
      MI.setDesc(get(Z80old::LD8ri));
      MI.findRegisterDefOperand(Z80old::F)->ChangeToImmediate(0);
    }
    break;
  case Z80old::CP16ao: {
      unsigned Reg = Opc == Z80old::HL;
      if (MBB.computeRegisterLiveness(&TRI, Reg, Next) !=
          MachineBasicBlock::LQR_Dead) {
        BuildMI(MBB, Next, DL, get(Z80old::ADD16ao), Reg)
  ...
  case Z80old::SUB16ao:
    expandPostRAPseudo(*BuildMI(MBB, MI, DL, get(Z80old::RCF)));
    MI.setDesc(get(Z80old::SBC16ao));
    break;
  case Z80old::CP16a0: {
      unsigned Reg = Opc == Z80old::HL;
      unsigned UndefReg = Opc == Z80old::BC;
      BuildMI(MBB, MI, DL, get(Z80old::ADD16ao),
              Reg).addReg(Reg).addReg(UndefReg, RegState::Undef);
      expandPostRAPseudo(*BuildMI(MBB, MI, DL, get(Z80old::RCF)));
      MI.setDesc(get(Opc == Z80old::SBC16ao));
  ...
  case Z80old::LD8rp: {
      ...
      if (Z80old::I8RegClass.contains(DstOp.getReg())) {
        BuildMI(MBB, MI, DL, get(Z80old::PUSH16AF));
        DstOp.setReg(Z80old::A);
        copyPhysReg(MBB, Next, DL, DstOp.getReg(), Z80old::A, true);
        BuildMI(MBB, Next, DL, get(Z80old::POP16AF));
      }
#endif // 0
      MI.setDesc(get(Opc == Z80old::LD8ro ? Z80old::LD8go : Z80old::LD8gp));
      break;
    }
  case Z80old::LD88rp:
  case Z80old::LD88ro: {
      //assert(!Subtarget.has16BitEZ80oldOps() &&
      //       "LD88rp/LD88ro is not used on the ez80old in 16-bit mode");
      unsigned NewOpc = Opc == Z80old::LD88rp ? Z80old::LD8rp : Z80old::LD8ro;
      MachineOperand &DstOp = MI.getOperand(0);
      const MachineOperand &AddrOp = MI.getOperand(1);
      unsigned OrigReg = DstOp.getReg();
      unsigned Reg = OrigReg;
      bool Index = Z80old::IR16RegClass.contains(Reg);
      bool Overlap = NewOpc == Z80old::LD8rp && Reg == Z80old::HL;
      unsigned ScratchReg;
      if (Index || Overlap) {
        Reg = Index ? Z80old::HL : Z80old::DE;
        ScratchReg = Reg;
        BuildMI(MBB, MI, DL, get(Z80old::PUSH16r))
        .addReg(ScratchReg, RegState::Undef);
      }
      MIB = BuildMI(MBB, MI, DL, get(NewOpc), TRI.getSubReg(Reg, Z80old::sub_low))
            .addReg(AddrOp.getReg());
      if (NewOpc == Z80old::LD8rp)
        BuildMI(MBB, MI, DL, get(Z80old::INC16r),
                AddrOp.getReg()).addReg(AddrOp.getReg());
      MI.setDesc(get(NewOpc));
      DstOp.setReg(TRI.getSubReg(Reg, Z80old::sub_high));
      if (NewOpc == Z80old::LD8ro) {
        MachineOperand &OffOp = MI.getOperand(2);
        MIB.addImm(OffOp.getImm());
        OffOp.setImm(OffOp.getImm() + 1);
      } else if (!AddrOp.isKill())
        BuildMI(MBB, Next, DL, get(Z80old::DEC16r),
                AddrOp.getReg()).addReg(AddrOp.getReg());
      if (Index)
        BuildMI(MBB, Next, DL, get(Z80old::EX16SP),
                ScratchReg).addReg(ScratchReg);
      else if (Overlap) {
        copyPhysReg(MBB, Next, DL, AddrOp.getReg(), ScratchReg, true);
      }
      if (Index || Overlap)
        BuildMI(MBB, Next, DL, get(Z80old::POP16r),
  ...
  case Z80old::LD8pr: {
      ...
      if (Z80old::I8RegClass.contains(SrcOp.getReg())) {
        BuildMI(MBB, MI, DL, get(Z80old::PUSH16AF));
        copyPhysReg(MBB, MI, DL, Z80old::A, SrcOp.getReg(), SrcOp.isKill());
        SrcOp.setReg(Z80old::A);
        SrcOp.setIsKill();
        BuildMI(MBB, Next, DL, get(Z80old::POP16AF));
      }
#endif // 0
      MI.setDesc(get(Opc == Z80old::LD8or ? Z80old::LD8og : Z80old::LD8pg));
      break;
    }
  case Z80old::LD88pr:
  case Z80old::LD88or: {
      //assert(!Subtarget.has16BitEZ80oldOps() &&
      //       "LD88pr/LD88or is not used on the ez80old in 16-bit mode");
      unsigned NewOpc = Opc == Z80old::LD88pr ? Z80old::LD8pr : Z80old::LD8or;
      const MachineOperand &AddrOp = MI.getOperand(0);
      MachineOperand &SrcOp = MI.getOperand(MI.getNumExplicitOperands() - 1);
      unsigned Reg = SrcOp.getReg();
      bool Index = Z80old::IR16RegClass.contains(Reg);
      bool Overlap = NewOpc == Z80old::LD8pr && Reg == Z80old::HL;
      unsigned ScratchReg = Z80old::HL;
      if (Index) {
        //unsigned SuperReg = TRI.getMatchingSuperReg(Reg, Z80old::sub_short,
        //                                            &Z80old::R24RegClass);
        BuildMI(MBB, MI, DL, get(Z80old::PUSH16r))
        .addReg(UseLEA || Overlap ? ScratchReg : Reg,
                RegState::Undef);
        //if (UseLEA)
        //  BuildMI(MBB, MI, DL, get(Z80old::LEA24ro), Z80old::UHL)
        //    .addReg(SuperReg).addImm(0);
        //else
        BuildMI(MBB, MI, DL, get(Z80old::EX16SP),
                ScratchReg).addReg(ScratchReg);
        Reg = Z80old::HL;
      } else if (Overlap) {
        BuildMI(MBB, MI, DL, get(Z80old::PUSH16AF));
      ...
      if (NewOpc == Z80old::LD8or) {
        ...
        BuildMI(MBB, Next, DL, get(Z80old::DEC16r),
                AddrOp.getReg()).addReg(AddrOp.getReg());
      MIB.addReg(TRI.getSubReg(Reg, Z80old::sub_low));
      Reg = TRI.getSubReg(Reg, Z80old::sub_high);
      if (Overlap) {
        copyPhysReg(MBB, MI, DL, Z80old::A, Reg, false);
        Reg = Z80old::A;
      }
      if (NewOpc == Z80old::LD8pr)
        BuildMI(MBB, MI, DL, get(Z80old::INC16r),
                AddrOp.getReg()).addReg(AddrOp.getReg());
      MI.setDesc(get(NewOpc));
      SrcOp.setReg(Reg);
  ...
  case Z80old::LD16rm:
    expandLoadStoreWord(&Z80old::AIR16RegClass, Z80old::LD16am,
                        &Z80old::OR16RegClass, Z80old::LD16om, MI, 0);
    break;
  case Z80old::LD16mr:
    expandLoadStoreWord(&Z80old::AIR16RegClass, Z80old::LD16ma,
                        &Z80old::OR16RegClass, Z80old::LD16mo, MI, 1);
    break;
  case Z80old::CALL16r: {
      const char *Symbol;
      switch (MIB->getOperand(0).getReg()) {
      default: llvm_unreachable("Unexpected indcall register");
      case Z80old::HL: Symbol = "_indcallhl"; break;
      case Z80old::IX: Symbol = "_indcallix"; break;
      case Z80old::IY: Symbol = "_indcall"; break;
      }
      MI.setDesc(get(Z80old::CALL16i));
      MI.getOperand(0).ChangeToES(Symbol);
      break;
    }
  case Z80old::EI_RETI:
    BuildMI(MBB, MI, DL, get(Z80old::EI));
    MI.setDesc(get(Z80old::RETI));
    break;
  case Z80old::TCRETURN16i:
    MI.setDesc(get(Z80old::JP16));
    break;
  case Z80old::TCRETURN16r:
    MI.setDesc(get(Z80old::JP16r));
    break;
  case Z80old::PUSH8r: {
      unsigned SrcReg8 = MI.getOperand(0).getReg();
      unsigned DstReg16 = llvm::getZ80oldSuperRegisterOrZero(SrcReg8);
      assert(DstReg16 && "Cannot push 8 bit register via 16 bit pair");
      MI.setDesc(get(Z80old::PUSH16r));
...
bool Z80oldInstrInfo::analyzeCompare(const MachineInstr &MI,
...
inline static bool isRedundantFlagInstr(MachineInstr &FI, unsigned SrcReg,
  ...
  if (ImmMask)
    return (FI.getOpcode() == Z80old::CP8ai && OI.getOpcode() == Z80old::SUB8ai) &&
           OI.getOperand(1).getImm() == ImmValue;
  else
    return (FI.getOpcode() == Z80old::CP8ar && OI.getOpcode() == Z80old::SUB8ar) &&
...
inline static bool isSZSettingInstr(MachineInstr &MI) {
  switch (MI.getOpcode()) {
  default: return false;
  case Z80old::INC8r:  case Z80old::INC8p:  case Z80old::INC8o:
  case Z80old::DEC8r:  case Z80old::DEC8p:  case Z80old::DEC8o:
  case Z80old::ADD8ar: case Z80old::ADD8ai: case Z80old::ADD8ap: case Z80old::ADD8ao:
  case Z80old::ADC8ar: case Z80old::ADC8ai: case Z80old::ADC8ap: case Z80old::ADC8ao:
  case Z80old::SUB8ar: case Z80old::SUB8ai: case Z80old::SUB8ap: case Z80old::SUB8ao:
  case Z80old::SBC8ar: case Z80old::SBC8ai: case Z80old::SBC8ap: case Z80old::SBC8ao:
  case Z80old::AND8ar: case Z80old::AND8ai: case Z80old::AND8ap: case Z80old::AND8ao:
  case Z80old::XOR8ar: case Z80old::XOR8ai: case Z80old::XOR8ap: case Z80old::XOR8ao:
  case Z80old:: OR8ar: case Z80old:: OR8ai: case Z80old:: OR8ap: case Z80old:: OR8ao:
  case Z80old::SBC16ao: case Z80old::NEG8:   case Z80old::ADC16ao:
  case Z80old::SUB16ao:
  case Z80old::RLC8r:  case Z80old::RLC8p:  case Z80old::RLC8o:
  case Z80old::RRC8r:  case Z80old::RRC8p:  case Z80old::RRC8o:
  case Z80old:: RL8r:  case Z80old:: RL8p:  case Z80old:: RL8o:
  case Z80old:: RR8r:  case Z80old:: RR8p:  case Z80old:: RR8o:
  case Z80old::SLA8r:  case Z80old::SLA8p:  case Z80old::SLA8o:
  case Z80old::SRA8r:  case Z80old::SRA8p:  case Z80old::SRA8o:
  case Z80old::SRL8r:  case Z80old::SRL8p:  case Z80old::SRL8o:
    return true;
  }
}
...
/// possible.
bool Z80oldInstrInfo::optimizeCompareInstr(MachineInstr &CmpInstr,
                                        unsigned SrcReg, unsigned SrcReg2,
  ...
  case Z80old::SUB8ai: CpOp = IsCmpZero ? Z80old::OR8ar : Z80old::CP8ai; break;
  case Z80old::SUB8ar: CpOp = Z80old::CP8ar; break;
  case Z80old::SUB8ap: CpOp = Z80old::CP8ap; break;
  case Z80old::SUB8ao: CpOp = Z80old::CP8ao; break;
  }
  if (CpOp) {
    int DeadDef = CmpInstr.findRegisterDefOperandIdx(Z80old::A, /*isDead*/true);
    if (DeadDef == -1) {
      return false;
    }
    // There is no use of the destination register, so we replace SUB with CP.
    CmpInstr.setDesc(get(CpOp));
    if (CpOp == Z80old::OR8ar) {
      CmpInstr.getOperand(0).ChangeToRegister(Z80old::A, false);
    } else {
  ...
  for (auto RI = ++I.getReverse(); RI != RE; ++RI) {
    ...
    if (Instr.modifiesRegister(Z80old::F, TRI)) {
      return false;
    }
  }
  ...
  for (++I; I != E; ++I) {
    ...
    bool ModifiesFlags = Instr.modifiesRegister(Z80old::F, TRI);
    bool UsesFlags = Instr.readsRegister(Z80old::F, TRI);
    ...
    if (IsCmpZero) {
      Z80old::CondCode OldCC;
      switch (Instr.getOpcode()) {
      default:
        OldCC = Z80old::COND_INVALID;
        break;
      case Z80old::JQCC:
        OldCC = static_cast<Z80old::CondCode>(Instr.getOperand(1).getImm());
        break;
      case Z80old::ADC8ar: case Z80old::ADC8ai: case Z80old::ADC8ap: case Z80old::ADC8ao:
      case Z80old::SBC8ar: case Z80old::SBC8ai: case Z80old::SBC8ap: case Z80old::SBC8ao:
      case Z80old::SBC16ao: case Z80old::ADC16ao:
        OldCC = Z80old::COND_C;
        break;
      }
      switch (OldCC) {
      default: break;
      case Z80old::COND_NC: case Z80old::COND_C:
      case Z80old::COND_PO: case Z80old::COND_PE:
        // CF or PV are used, we can't perform this optimization.
        return false;
      }
    }
    if (ModifiesFlags || Instr.killsRegister(Z80old::F, TRI)) {
  ...
  for (; i != e; ++i) {
    ...
    if (MO.isReg() && MO.isDef() && MO.getReg() == Z80old::F) {
      MO.setIsDead(false);
      break;
    }
  }
  ...
  case Z80old::SUB8ai:
    ...
    if (!CmpInstr.getOperand(0).getImm()) {
      CmpInstr.setDesc(get(Z80old::OR8ar));
      CmpInstr.getOperand(0).ChangeToRegister(Z80old::A, /*isDef=*/false);
      return true;
    }
    LLVM_FALLTHROUGH;
  case Z80old::SUB8ar:
  case Z80old::SUB8ap:
  case Z80old::SUB8ao: {
      if (!CmpInstr.registerDefIsDead(Z80old::A)) {
        return false;
      }
      ...
      case Z80old::SUB8ai: NewOpcode = Z80old::CP8ai; break;
      case Z80old::SUB8ar: NewOpcode = Z80old::CP8ar; break;
      case Z80old::SUB8ap: NewOpcode = Z80old::CP8ap; break;
      case Z80old::SUB8ao: NewOpcode = Z80old::CP8ao; break;
      }
      CmpInstr.setDesc(get(NewOpcode));
      //CmpInstr.findRegisterDefOperand(Z80old::A)->setIsDead(false);
      //BuildMI(*CmpInstr.getParent(), ++MachineBasicBlock::iterator(CmpInstr), CmpInstr.getDebugLoc(), get(TargetOpcode::COPY), SrcReg).addReg(Z80old::A, RegState::Kill);
      return true;
    }
  }
}
...
MachineInstr *
Z80oldInstrInfo::foldMemoryOperandImpl(MachineFunction &MF, MachineInstr &MI,
  ...
  if (Ops.size() == 1 && Ops[0] == 1 && MI.isFullCopy()) {
    ...
    if (TargetRegisterInfo::isPhysicalRegister(DstReg)) {
      unsigned Opc;
      if (Z80old::RR8RegClass.contains(DstReg)) {
        Opc = Z80old::LD8ro;
      } else {
        assert(false/*(Z80old::R16RegClass)
