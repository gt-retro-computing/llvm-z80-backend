//===-- Z80oldISelLowering.h - Z80old DAG Lowering Interface ----------*- C++ -*-===//
//
...
#ifndef LLVM_LIB_TARGET_Z80old_Z80oldISELLOWERING_H
#define LLVM_LIB_TARGET_Z80old_Z80oldISELLOWERING_H
...
enum NodeType : unsigned {
  ...
  /// This operation represents an abstract Z80old call instruction, which
  ...
  /// BRCOND - Z80old conditional branch.  The first operand is the chain, the
  /// second is the block to branch to if the condition is true, the third is
  /// the condition, and the fourth is the flag operand.
  BRCOND,

  /// SELECT - Z80old select - This selects between a true value and a false
...
} // end Z80oldISD namespace

namespace Z80old {
bool isCalleePop(CallingConv::ID CallingConv,
                 /*bool is64Bit,*/ bool IsVarArg, bool GuaranteeTCO);
} // end namespace Z80old

//===----------------------------------------------------------------------===//
//  Z80old Implementation of the TargetLowering interface
class Z80oldTargetLowering final : public TargetLowering {
  /// Keep a reference to the Z80oldSubtarget around so that we can
  /// make the right decision when generating code for different targets.
  const Z80oldSubtarget &Subtarget;

public:
  explicit Z80oldTargetLowering(const Z80oldTargetMachine &TM,
  ...
  /// type Ty1 to type Ty2. e.g. On z80old it's free to truncate an i16 value in
  ...
  /// does not necessarily apply to truncate instructions. e.g. on ez80old,
  ...
  /// VT2. e.g. on ez80old, it's profitable to narrow from i24 to i8 but not from
  /// i24 to i16.
  ...
  /// given node type. e.g. On ez80old i16 is legal, but undesirable since i16
