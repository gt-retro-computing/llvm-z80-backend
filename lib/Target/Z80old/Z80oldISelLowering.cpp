//===-- Z80oldISelLowering.cpp - Z80old DAG Lowering Implementation -------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines the interfaces that Z80old uses to lower LLVM code into a
// selection DAG.
//
//===----------------------------------------------------------------------===//
...
bool Z80old::isCalleePop(CallingConv::ID CallingConv,
...
Z80oldTargetLowering::Z80oldTargetLowering(const Z80oldTargetMachine &TM,
                                     const Z80oldSubtarget &STI)
  : TargetLowering(TM), Subtarget(STI) {
  //bool HasEZ80oldOps = Subtarget.hasEZ80oldOps(), Is24Bit = Subtarget.is24Bit();
  MVT PtrVT = MVT::getIntegerVT(8 * TM.getPointerSize(0));

  setSchedulingPreference(Sched::RegPressure);

  // Set up the register classes.
  addRegisterClass(MVT::i8, &Z80old::RR8RegClass);
  addRegisterClass(MVT::i16, &Z80old::R16RegClass);
  //if (Is24Bit)
  //  addRegisterClass(MVT::i24, &Z80old::R24RegClass);
  ...
  setStackPointerRegisterToSaveRestore(Z80old::SPS);
...
SDValue Z80oldTargetLowering::EmitOffset(int64_t Amount, const SDLoc &DL,
                                      SDValue Op, SelectionDAG &DAG) const {
  EVT VT = Op.getValueType();
  return DAG.getNode(ISD::ADD, DL, VT, Op, DAG.getConstant(Amount, DL, VT));
}

SDValue Z80oldTargetLowering::EmitNegate(const SDLoc &DL, SDValue Op,
                                      SelectionDAG &DAG) const {
  EVT VT = Op.getValueType();
  return DAG.getNode(ISD::SUB, DL, VT, DAG.getConstant(0, DL, VT), Op);
}

SDValue Z80oldTargetLowering::EmitFlipSign(const SDLoc &DL, SDValue Op,
                                        SelectionDAG &DAG) const {
  EVT VT = Op.getValueType();
...
SDValue Z80oldTargetLowering::EmitPair(const SDLoc &DL, SDValue Hi, SDValue Lo,
                                    SelectionDAG &DAG) const {
  assert(Hi.getValueType() == MVT::i8 && Lo.getValueType() == MVT::i8 &&
         "Can only emit a pair of i8 to i16");
  SDValue Ops[] = {
    DAG.getTargetConstant(Z80old::GR16RegClassID, DL, MVT::i32),
    Hi, DAG.getTargetConstant(Z80old::sub_high, DL, MVT::i32),
    Lo, DAG.getTargetConstant(Z80old::sub_low, DL, MVT::i32),
  };
  SDNode *Res = DAG.getMachineNode(TargetOpcode::REG_SEQUENCE, DL, MVT::i16, Ops);
  return SDValue(Res, 0);
}

SDValue Z80oldTargetLowering::EmitLow(SDValue Op, SelectionDAG &DAG) const {
  return DAG.getNode(ISD::TRUNCATE, SDLoc(Op), MVT::i8, Op);
}

SDValue Z80oldTargetLowering::EmitHigh(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  return EmitLow(DAG.getNode(ISD::SRL, DL, Op.getValueType(), Op,
                             DAG.getConstant(8, DL, MVT::i8)), DAG);
}

SDValue Z80oldTargetLowering::EmitSignToCarry(SDValue Op,
                                           SelectionDAG &DAG) const {
  SDLoc DL(Op);
  EVT VT = Op.getValueType();
#if 0
  if (VT == MVT::i24) {
    if (Op.hasOneUse() && ISD::isNormalLoad(Op.getNode())) {
      ...
      return DAG.getNode(Z80oldISD::ADD, DL, DAG.getVTList(VT, MVT::i8), Op, Op)
             .getValue(1);
  } else
#endif // 0
    if (VT == MVT::i16) {
      Op = DAG.getTargetExtractSubreg(Z80old::sub_high, DL, MVT::i8, Op);
    }
  assert(Op.getValueType() == MVT::i8 && "Unexpected type!");
  return DAG.getNode(Z80oldISD::SLA, DL, DAG.getVTList(MVT::i8, MVT::i8), Op)
         .getValue(1);
}

// Legalize Types Helpers

void Z80oldTargetLowering::ReplaceNodeResults(SDNode *N,
...
SDValue Z80oldTargetLowering::LowerAddSub(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue Z80oldTargetLowering::LowerBitwise(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  unsigned Opc = Op.getOpcode();
  ...
  if (((Known.Zero | Known.One) & 0x00FF00) != 0x00FF00) {
    SDValue LHSHi = DAG.getTargetExtractSubreg(Z80old::sub_high, DL, MVT::i8, LHS);
    SDValue RHSHi = DAG.getTargetExtractSubreg(Z80old::sub_high, DL, MVT::i8, RHS);
    ResHi = DAG.getNode(Opc, DL, MVT::i8, LHSHi, RHSHi);
    Known.One &= ~0x00FF00;
  }
  if (((Known.Zero | Known.One) & 0x0000FF) != 0x0000FF) {
    SDValue LHSLo = DAG.getTargetExtractSubreg(Z80old::sub_low, DL, MVT::i8, LHS);
    SDValue RHSLo = DAG.getTargetExtractSubreg(Z80old::sub_low, DL, MVT::i8, RHS);
  ...
  if (ResHi) {
    Res = DAG.getTargetInsertSubreg(Z80old::sub_high, DL, VT, Res, ResHi);
  }
  if (ResLo) {
    Res = DAG.getTargetInsertSubreg(Z80old::sub_low,  DL, VT, Res, ResLo);
  }
  return Res;
}

SDValue Z80oldTargetLowering::LowerShift(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  unsigned Opc = Op.getOpcode();
  ...
  if (ConstantSDNode *Shift = dyn_cast<ConstantSDNode>(Op.getOperand(1))) {
    ...
    if (Opc == ISD::SRA && Amt == VT.getSizeInBits() - 1) {
      return DAG.getNode(Z80oldISD::SEXT, DL, VT, EmitSignToCarry(Val, DAG));
    }
    ...
    case 8:
      ...
      case ISD::SHL: Opc = Z80oldISD::SLA; break;
      case ISD::SRA: Opc = Z80oldISD::SRA; break;
      case ISD::SRL: Opc = Z80oldISD::SRL; break;
      case ISD::ROTL: Opc = Z80oldISD::RLC; break;
      case ISD::ROTR: Opc = Z80oldISD::RRC; break;
      }
      while (Amt--) {
    ...
    case 16:
      if (Amt >= 8) {
        Val = DAG.getTargetExtractSubreg(
                Opc == ISD::SHL ? Z80old::sub_low : Z80old::sub_high, DL, MVT::i8, Val);
      ...
      if (Opc != ISD::SHL && (!OptSize || Amt == 1)) {
        unsigned Opc0 = Opc == ISD::SRA ? Z80oldISD::SRA : Z80oldISD::SRL;
        SDValue Val0 = DAG.getTargetExtractSubreg(Z80old::sub_high, DL,
                                                  MVT::i8, Val);
        unsigned Opc1 = Z80oldISD::RR;
        SDValue Val1 = DAG.getTargetExtractSubreg(Z80old::sub_low, DL,
                                                  MVT::i8, Val);
        while (Amt--) {
    ...
    case 24:
      if (Opc == ISD::SHL) {
        ...
        while (Amt--)
          Val = DAG.getNode(Z80oldISD::ADD, DL,
...
SDValue Z80oldTargetLowering::LowerAnyExtend(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getTargetInsertSubreg(
           ValVT == MVT::i16 ? Z80old::sub_short : Z80old::sub_low, DL, VT,
           DAG.getUNDEF(VT), Val);
}

SDValue Z80oldTargetLowering::LowerZeroExtend(SDValue Op,
                                           SelectionDAG &DAG) const {
...
SDValue Z80oldTargetLowering::LowerSignExtend(SDValue Op,
                                           SelectionDAG &DAG) const {
  ...
  if (VT == MVT::i16) {
    ...
    return DAG.getTargetInsertSubreg(
             Z80old::sub_high, DL, VT, DAG.getNode(ISD::ANY_EXTEND, DL, VT, Val),
             DAG.getNode(Z80oldISD::SEXT, DL, ValVT, Sign));
  }
  //assert(VT == MVT::i24 && (ValVT == MVT::i16 || ValVT == MVT::i8) &&
  //       "Unexpected sign extend");
  return DAG.getTargetInsertSubreg(
           ValVT == MVT::i16 ? Z80old::sub_short : Z80old::sub_low, DL, VT,
           DAG.getNode(Z80oldISD::SEXT, DL, VT, Sign), Val);
}

SDValue Z80oldTargetLowering::LowerMul(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue Res = DAG.getZExtOrTrunc(
                  DAG.getNode(Z80oldISD::MLT, DL, MVT::i16,
...
SDValue Z80oldTargetLowering::LowerGlobalAddress(GlobalAddressSDNode *Node,
                                              SelectionDAG &DAG) const {
  SDLoc DL(Node);
  auto PtrVT = getPointerTy(DAG.getDataLayout());
  SDValue Res = DAG.getTargetGlobalAddress(Node->getGlobal(), DL, PtrVT,
                                           Node->getOffset());
  return DAG.getNode(Z80oldISD::Wrapper, DL, PtrVT, Res);
}

SDValue Z80oldTargetLowering::LowerExternalSymbol(ExternalSymbolSDNode *Node,
                                               SelectionDAG &DAG) const {
  SDLoc DL(Node);
  auto PtrVT = getPointerTy(DAG.getDataLayout());
...
SDValue Z80oldTargetLowering::LowerBlockAddress(BlockAddressSDNode *Node,
                                             SelectionDAG &DAG) const {
  SDLoc DL(Node);
  auto PtrVT = getPointerTy(DAG.getDataLayout());
...
SDValue Z80oldTargetLowering::LowerLoad(LoadSDNode *Node,
                                     SelectionDAG &DAG) const {
  SDLoc DL(Node);
  ...
  bool Split = Z80old::splitReg(MMO->getSize(), MVT::i8, // MVT::i16,
                             RC, LoTy, LoIdx, HiTy, HiIdx, HiOff
                             /*Subtarget.hasEZ80oldOps()*/);
...
SDValue Z80oldTargetLowering::LowerStore(StoreSDNode *Node,
                                      SelectionDAG &DAG) const {
  SDLoc DL(Node);
  SDValue Ch = Node->getChain();
  SDValue Ptr = Node->getBasePtr();
...
SDValue Z80oldTargetLowering::LowerVAStart(SDValue Op, SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  Z80oldMachineFunctionInfo *FuncInfo = MF.getInfo<Z80oldMachineFunctionInfo>();
  SDLoc DL(Op);
...
SDValue Z80oldTargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) const {
...
SDValue Z80oldTargetLowering::EmitCmp(SDValue LHS, SDValue RHS, SDValue &TargetCC,
  ...
  Z80old::CondCode TCC = Z80old::COND_INVALID;
  unsigned Opc = Z80oldISD::SUB;
  switch (CC) {
  ...
  case Z80old::COND_NC:
    ...
    if (VT != MVT::i8 && Const) {
      Opc = Z80oldISD::ADD;
      TCC = Z80old::GetOppositeBranchCondition(TCC);
      ConstVal = -ConstVal;
      if (ConstVal == SignVal) {
        RHS = LHS;
        Const = nullptr;
...
SDValue Z80oldTargetLowering::EmitExtractSubreg(unsigned Idx, const SDLoc &DL,
                                             SDValue Op,
                                             SelectionDAG &DAG) const {
...
SDValue Z80oldTargetLowering::EmitInsertSubreg(unsigned Idx, const SDLoc &DL,
                                            MVT VT, SDValue Op,
                                            SelectionDAG &DAG) const {
  const TargetRegisterInfo *TRI = Subtarget.getRegisterInfo();
...
SDValue Z80oldTargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(Z80oldISD::BRCOND, DL, MVT::Other,
                     Chain, Dest, TargetCC, Flags);
}

SDValue Z80oldTargetLowering::LowerSETCC(SDValue Op, SelectionDAG &DAG) const {
  ...
  EVT VT = Op.getValueType();
  return DAG.getNode(Z80oldISD::SELECT, DL, DAG.getVTList(VT, MVT::Glue),
                     DAG.getConstant(1, DL, VT), DAG.getConstant(0, DL, VT),
                     TargetCC, Flags);
}

SDValue Z80oldTargetLowering::LowerSELECT_CC(SDValue Op, SelectionDAG &DAG) const {
  SDValue LHS = Op.getOperand(0);
  SDValue RHS = Op.getOperand(1);
  ...
  return DAG.getNode(Z80oldISD::SELECT, DL,
                     DAG.getVTList(Op.getValueType(), MVT::Glue), TV, FV,
                     TargetCC, Flag);
}

SDValue Z80oldTargetLowering::LowerLibCall(
...
bool Z80oldTargetLowering::isOffsetFoldingLegal(
...
bool Z80oldTargetLowering::isLegalAddressingMode(const DataLayout &DL,
...
bool Z80oldTargetLowering::isLegalICmpImmediate(int64_t Imm) const {
  return isInt<8>(Imm);
}
bool Z80oldTargetLowering::isLegalAddImmediate(int64_t Imm) const {
  return isInt<8>(Imm);
}

bool Z80oldTargetLowering::isTruncateFree(Type *Ty1, Type *Ty2) const {
  if (!Ty1->isIntegerTy() || !Ty2->isIntegerTy()) {
    return false;
  }
  return Ty1->getPrimitiveSizeInBits() > Ty2->getPrimitiveSizeInBits();
}
bool Z80oldTargetLowering::isTruncateFree(EVT VT1, EVT VT2) const {
...
#if 0
bool Z80oldTargetLowering::isZExtFree(Type *Ty1, Type *Ty2) const {
  // ez80old implicitly zero-extends 16-bit results in 24-bit registers.
  return Ty1->isIntegerTy(16) && Ty2->isIntegerTy(24) && Subtarget.is24Bit();
}
bool Z80oldTargetLowering::isZExtFree(EVT VT1, EVT VT2) const {
  // ez80old implicitly zero-extends 16-bit results in 24-bit registers.
  return VT1 == MVT::i16 && VT2 == MVT::i24 && Subtarget.is24Bit();
}

bool Z80oldTargetLowering::isNarrowingProfitable(EVT VT1, EVT VT2) const {
...
bool Z80oldTargetLowering::shouldConvertConstantLoadToIntImm(const APInt &Imm,
                                                          Type *Ty) const {
...
SDValue Z80oldTargetLowering::combineCopyFromReg(SDNode *N,
...
SDValue Z80oldTargetLowering::combineStore(StoreSDNode *N,
                                        DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
  MachineFunction &MF = DAG.getMachineFunction();
  MachineFrameInfo &MFI = MF.getFrameInfo();
...
SDValue Z80oldTargetLowering::combineINSERT_SUBREG(SDNode *N,
                                                DAGCombinerInfo &DCI) const {
  return SDValue();
}

// This code is from DAGCombiner::visitADDC
SDValue Z80oldTargetLowering::combineADD(SDNode *N, DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
...
SDValue Z80oldTargetLowering::combineSUB(SDNode *N, DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
  EVT VT = N->getValueType(0);
  SDValue N0 = N->getOperand(0);
  SDValue N1 = N->getOperand(1);
...
static SDValue combineMul(SDNode *N, SelectionDAG &DAG,
  ...
  if (auto C1 = dyn_cast<ConstantSDNode>(N1)) {
    ...
    if (Odd) {
      Double = DAG.getNode(Z80oldISD::ADD, DL, VT, Double, N0);
    }
    return Double;
  }
  return SDValue();
...
static SDValue implicitlyClearTop(SDValue &Op, SelectionDAG &DAG) {
  ...
  case ISD::ADD: NewOpc = Z80oldISD::ADD; break;
  case ISD::SUB: NewOpc = Z80oldISD::SUB; break;
  }
  ...
  return SDValue(DAG.getMachineNode(TargetOpcode::SUBREG_TO_REG, DL,
                                    ...
                                    DAG.getTargetConstant(Z80old::sub_short,
                                                          DL, MVT::i32)), 0);
}
#endif // 0
...
static SDValue combineTruncate(SDNode *N, SelectionDAG &DAG) {
  ...
  if (VT == MVT::i8 && Val.hasOneUse())
    ...
    case ISD::SRA:
      ...
      break;
    case Z80oldISD::SEXT:
      return DAG.getNode(Z80oldISD::SEXT, DL, VT, Val.getOperand(0));
    }
  return SDValue();
}
...
static SDValue combineSub(SDNode *N, TargetLowering::DAGCombinerInfo &DCI) {
  ...
  if (Val0.use_empty())
    return DCI.CombineTo(N, Val0,
                         DAG.getNode(Z80oldISD::CP, DL, N->getValueType(1),
...
static SDValue combineSExt(SDNode *N, SelectionDAG &DAG,
  ...
  //  return DAG.getNode(ISD::TRUNCATE, DL, VT,
  //                     DAG.getNode(Z80oldISD::SEXT, DL, MVT::i24, N0));
  return SDValue();
}

SDValue Z80oldTargetLowering::PerformDAGCombine(SDNode *N,
                                             DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
  ...
  case Z80oldISD::SUB:      return combineSub(N, DCI);
  case Z80oldISD::SEXT:     return combineSExt(N, DCI.DAG, Subtarget);
  }
//switch (N->getOpcode()) {
...
//case Z80oldISD::ADD:                  return combineADD(N, DCI);
//case Z80oldISD::SUB:                  return combineSUB(N, DCI);
//}
  return SDValue();
}
...
/// and it is 'desirable' to use the type for the given node type. e.g. On ez80old
/// i16 is legal, but undesirable since i16 instruction encodings are longer and
/// slower.
bool Z80oldTargetLowering::isTypeDesirableForOp(unsigned Opc, EVT VT) const {
  if (!isTypeLegal(VT)) {
    return false;
  }
#if 0
...
bool Z80oldTargetLowering::isDesirableToShrinkOp(unsigned Opc, EVT SrcVT,
                                              EVT DstVT) const {
  ...
  case ISD::SUBE:
    ...
    // $TODO: Are these for Z80old as well?
...
/// type by reference.
bool Z80oldTargetLowering::IsDesirableToPromoteOp(SDValue Op, EVT &PVT) const {
#if 0
...
MachineBasicBlock *
Z80oldTargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
  ...
  case Z80old::Select8:
  case Z80old::Select16:
    //case Z80old::Select24:
...
#if 1
void Z80oldTargetLowering::AdjustInstrPostInstrSelection(MachineInstr &MI,
                                                      SDNode *Node) const {
  switch (MI.getOpcode()) {
  default: llvm_unreachable("Unexpected instr type to insert");
  case Z80old::ADJCALLSTACKUP16:
  //case Z80old::ADJCALLSTACKUP24:
  case Z80old::ADJCALLSTACKDOWN16:
    //case Z80old::ADJCALLSTACKDOWN24:
    return AdjustAdjCallStack(MI);
  }
}

void Z80oldTargetLowering::AdjustAdjCallStack(MachineInstr &MI) const {
  //bool Is24Bit = MI.getOpcode() == Z80old::ADJCALLSTACKUP24 ||
  //               MI.getOpcode() == Z80old::ADJCALLSTACKDOWN24;
  assert((/*Is24Bit ||*/ MI.getOpcode() == Z80old::ADJCALLSTACKUP16 ||
                         MI.getOpcode() == Z80old::ADJCALLSTACKDOWN16) &&
         "Unexpected opcode");
  MachineRegisterInfo &MRI = MI.getParent()->getParent()->getRegInfo();
  unsigned Reg = MRI.createVirtualRegister(&Z80old::AIR16RegClass);
...
MachineBasicBlock *
Z80oldTargetLowering::EmitLoweredSub(MachineInstr &MI,
                                  MachineBasicBlock *BB) const {
  //bool Is24Bit = MI.getOpcode() == Z80old::SUB24ao;
  assert((/*Is24Bit ||*/ MI.getOpcode() == Z80old::SUB16ao) && "Unexpected opcode");
  const TargetInstrInfo *TII = Subtarget.getInstrInfo();
  DebugLoc DL = MI.getDebugLoc();
  LLVM_DEBUG(BB->dump());
  BuildMI(*BB, MI, DL, TII->get(Z80old::RCF));
  BuildMI(*BB, MI, DL, TII->get(Z80old::SBC16ao))
...
MachineBasicBlock *
Z80oldTargetLowering::EmitLoweredCmp(MachineInstr &MI,
                                  MachineBasicBlock *BB) const {
  //bool Is24Bit = MI.getOpcode() == Z80old::CP24ao;
  ...
  BuildMI(*BB, MI, DL, TII->get(Z80old::SBC16ao))
  .addReg(MI.getOperand(0).getReg());
  BuildMI(*BB, MI, DL, TII->get(Z80old::ADD16ao),
          Z80old::HL).addReg(Z80old::HL)
  .addReg(MI.getOperand(0).getReg());
  MI.eraseFromParent();
  LLVM_DEBUG(BB->dump());
  return BB;
...
MachineBasicBlock *
Z80oldTargetLowering::EmitLoweredCmp0(MachineInstr &MI,
                                   MachineBasicBlock *BB) const {
  //bool Is24Bit = MI.getOpcode() == Z80old::CP24a0;
...
MachineBasicBlock *
Z80oldTargetLowering::EmitLoweredSelect(MachineInstr &MI,
                                     MachineBasicBlock *BB) const {
  const TargetInstrInfo *TII = Subtarget.getInstrInfo();
  DebugLoc DL = MI.getDebugLoc();
  ...
  BuildMI(BB, DL, TII->get(Z80old::JQCC)).addMBB(copy1MBB)
  ...
  //  ...
  BB = copy1MBB;
  BuildMI(*BB, BB->begin(), DL, TII->get(Z80old::PHI),
...
MachineBasicBlock *Z80oldTargetLowering::EmitLoweredSExt(
...
#include "Z80oldGenCallingConv.inc"

CCAssignFn *Z80oldTargetLowering::getCCAssignFn(CallingConv::ID CallConv) const {
  //bool Is24Bit = Subtarget.is24Bit();
  switch (CallConv) {
  ...
  case CallingConv::PreserveAll:
    ...
    //  return CC_EZ80old_LC_AB;
    //case CallingConv::Z80_LibCall_AC:
    //  return CC_EZ80old_LC_AC;
    //case CallingConv::Z80_LibCall_BC:
    //  return CC_EZ80old_LC_BC;
    //case CallingConv::Z80_LibCall_C:
    //  return CC_EZ80old_LC_C;
    //case CallingConv::Z80_LibCall_L:
    //  return CC_EZ80old_LC_L;
  }
}
CCAssignFn *Z80oldTargetLowering::getRetCCAssignFn(CallingConv::ID CallConv)
const {
  //bool Is24Bit = Subtarget.is24Bit();
  switch (CallConv) {
  default: llvm_unreachable("Unsupported calling convention!");
  ...
  case CallingConv::Z80_LibCall_BC:
  case CallingConv::Z80_LibCall_C:
    return RetCC_Z80old_C;
    //case CallingConv::Z80_LibCall_L:
    //  return RetCC_EZ80old_LC_L;
  }
}

SDValue Z80oldTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  for (unsigned I = ArgLocs.size(); I; --I) {
    ...
    unsigned Reg = VA.isRegLoc() ? VA.getLocReg() : Z80old::NoRegister;
    ...
    if (OA.Flags.isSplitEnd() && !OA.Flags.isZExt() && !OA.Flags.isSExt()) {
      ...
      if (LocVT.getStoreSize() != RegBytes) {
        ...
        case 1: Idx = Z80old::sub_low;   break;
        case 2: Idx = Z80old::sub_short; break;
        }
    ...
    if (VA.isRegLoc()) {
      ...
      Chain = DAG.getMemIntrinsicNode(
                Z80oldISD::PUSH, DL, DAG.getVTList(MVT::Other),
  ...
  if (IsTailCall) {
    ...
    return DAG.getNode(Z80oldISD::TC_RETURN, DL, NodeTys, Ops);
  }

  // Returns a chain and a flag for retval copy to use.
  Chain = DAG.getNode(Z80oldISD::CALL, DL, NodeTys, Ops);
  InFlag = Chain.getValue(1);
...
bool Z80oldTargetLowering::IsEligibleForTailCallOptimization(
...
llvm::EVT llvm::Z80oldTargetLowering::getTypeForExtReturn(
  LLVMContext &Context, EVT VT, ISD::NodeType ExtendKind) const {
  return getRegisterType(Context, MVT::i8);
}

SDValue Z80oldTargetLowering::LowerReturn(SDValue Chain,
  ...
  unsigned Opc = Z80oldISD::RET_FLAG;
  if (F.hasFnAttribute("interrupt"))
    Opc = F.getFnAttribute("interrupt").getValueAsString() == "NMI" ?
          Z80oldISD::RETN_FLAG : Z80oldISD::RETI_FLAG;
...
///
SDValue
Z80oldTargetLowering::LowerCallResult(SDValue Chain, SDValue InFlag,
                                   CallingConv::ID CallConv, bool IsVarArg,
...
SDValue
Z80oldTargetLowering::LowerMemArgument(SDValue Chain, CallingConv::ID CallConv,
                                    const SmallVectorImpl<ISD::InputArg> &Ins,
...
SDValue Z80oldTargetLowering::LowerFormalArguments(
  ...
  for (unsigned I = 0, InsIndex = 0, E = ArgLocs.size(); I != E;
    ...
    if (VA.isRegLoc()) {
      ...
      if (VA.needsCustom()) {
        ...
        if (RegVT == MVT::i8) {
          RC = &Z80old::GR8RegClass;
        } else if (RegVT == MVT::i16)
          RC = &Z80old::GR16RegClass;
#if 0
  ...
  if (Z80old::isCalleePop(CallConv, /*Is64Bit,*/ isVarArg,
                       MF.getTarget().Options.GuaranteedTailCallOpt)) {
...
const char *Z80oldTargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch ((Z80oldISD::NodeType)Opcode) {
  case Z80oldISD::FIRST_NUMBER: break;
  case Z80oldISD::Wrapper:      return "Z80oldISD::Wrapper";
  case Z80oldISD::RLC:          return "Z80oldISD::RLC";
  case Z80oldISD::RRC:          return "Z80oldISD::RRC";
  case Z80oldISD::RL:           return "Z80oldISD::RL";
  case Z80oldISD::RR:           return "Z80oldISD::RR";
  case Z80oldISD::SLA:          return "Z80oldISD::SLA";
  case Z80oldISD::SRA:          return "Z80oldISD::SRA";
  case Z80oldISD::SRL:          return "Z80oldISD::SRL";
  case Z80oldISD::INC:          return "Z80oldISD::INC";
  case Z80oldISD::DEC:          return "Z80oldISD::DEC";
  case Z80oldISD::ADD:          return "Z80oldISD::ADD";
  case Z80oldISD::ADC:          return "Z80oldISD::ADC";
  case Z80oldISD::SUB:          return "Z80oldISD::SUB";
  case Z80oldISD::SBC:          return "Z80oldISD::SBC";
  case Z80oldISD::AND:          return "Z80oldISD::AND";
  case Z80oldISD::XOR:          return "Z80oldISD::XOR";
  case Z80oldISD::OR:           return "Z80oldISD::OR";
  case Z80oldISD::CP:           return "Z80oldISD::CP";
  case Z80oldISD::TST:          return "Z80oldISD::TST";
  case Z80oldISD::MLT:          return "Z80oldISD::MLT";
  case Z80oldISD::SEXT:         return "Z80oldISD::SEXT";
  case Z80oldISD::CALL:         return "Z80oldISD::CALL";
  case Z80oldISD::RET_FLAG:     return "Z80oldISD::RET_FLAG";
  case Z80oldISD::RETN_FLAG:    return "Z80oldISD::RETN_FLAG";
  case Z80oldISD::RETI_FLAG:    return "Z80oldISD::RETI_FLAG";
  case Z80oldISD::TC_RETURN:    return "Z80oldISD::TC_RETURN";
  case Z80oldISD::BRCOND:       return "Z80oldISD::BRCOND";
  case Z80oldISD::SELECT:       return "Z80oldISD::SELECT";
  case Z80oldISD::POP:          return "Z80oldISD::POP";
  case Z80oldISD::PUSH:         return "Z80oldISD::PUSH";
  }
  return nullptr;
}

EVT Z80oldTargetLowering::getSetCCResultType(const DataLayout &DL,
                                          LLVMContext &Context,
                                          EVT VT) const {
  assert(!VT.isVector() && "No default SetCC type for vectors!");
  return MVT::i8;
}
MVT::SimpleValueType Z80oldTargetLowering::getCmpLibcallReturnType() const {
  return MVT::Other;
}
