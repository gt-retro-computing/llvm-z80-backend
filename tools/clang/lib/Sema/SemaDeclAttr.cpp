...
static void handleAnyZ80InterruptAttr(Sema &S, Decl *D,
                                      const AttributeList &AL) {
  // Only one optional argument permitted.
  if (AL.getNumArgs() > 1) {
    S.Diag(AL.getLoc(), diag::err_attribute_too_many_arguments)
  ...
  if (!getFunctionOrMethodResultType(D)->isVoidType()) {
    S.Diag(getFunctionOrMethodResultSourceRange(D).getBegin(),
           diag::err_anyz80_interrupt_attribute)
        << (S.Context.getTargetInfo().getTriple().getArch() == llvm::Triple::z80
                ? 0
                : 1)
        << 0;
    return;
  ...
  if (getFunctionOrMethodNumParams(D) != 0) {
    S.Diag(D->getLocStart(), diag::err_anyz80_interrupt_attribute)
        << (S.Context.getTargetInfo().getTriple().getArch() == llvm::Triple::z80
                ? 0
                : 1)
        << 1;
  ...
  D->addAttr(::new (S.Context) AnyZ80InterruptAttr(
      AL.getLoc(), S.Context, Kind, AL.getAttributeSpellingListIndex()));
}
